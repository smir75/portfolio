...
export default function HUDPanel({
  ...
  // Animation légère (opacité + bande scan)
  useFrame(({ clock }) => {
    const k = kRef.current ?? 0;
    ...
    if (scan.current) {
      scan.current.position.y =
        (type === "totem" ? 0.35 : 0.22) * Math.sin(t * 1.4);
      scan.current.material.opacity = (highContrast ? 0.3 : 0.22) * k;
    }
  });

  // Interaction
  const handleClick = () => { if ((kRef.current ?? 0) >= 0.5) onClick?.(); };
  const commonProps = {
    onClick: handleClick,
  ...
  if (type === "round") {
    return (
      <Billboard follow distanceFactor={8} {...commonProps} onPointerDown={handleClick}>
        <mesh ref={plane}>
        ...
        <mesh position={[0, 0, -0.001]}>
          ...
          <meshBasicMaterial
            color={color}
            transparent
            opacity={highContrast ? 0.45 : 0.35}
            depthTest={false}
            depthWrite={false}
          />
        </mesh>
      ...
      <Billboard follow distanceFactor={8} {...commonProps}>
        ...
        <mesh position={[0, 0, -0.001]}>
          ...
          <meshBasicMaterial
            color={color}
            transparent
            opacity={highContrast ? 0.36 : 0.28}
            depthTest={false}
            depthWrite={false}
          />
        </mesh>
    ...
    <Billboard follow distanceFactor={8} {...commonProps}>
      ...
      <mesh position={[0, 0, -0.001]} rotation={[0, 0, Math.PI / 40]}>
        ...
        <meshBasicMaterial
          color={color}
          transparent
          opacity={highContrast ? 0.26 : 0.18}
          depthTest={false}
          depthWrite={false}
        />
      </mesh>
