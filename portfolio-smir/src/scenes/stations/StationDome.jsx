// src/scenes/stations/StationDome.jsx
  ...
  return (
    <group position={s.pos} quaternion={q}>
      <mesh position={[0, -0.15, 0]}>
        <torusGeometry args={[0.6, 0.06, 12, 48]} />
      ...
      <mesh position={[0, 0.1, 0]}>
        ...
        <meshPhysicalMaterial
          color={highContrast ? "#cfe6ff" : "#b6d7ff"}
          transmission={0.85}
          transparent
          roughness={0.15}
          thickness={0.15}
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <mesh key={i} position={[Math.cos((i/6)*Math.PI*2)*0.9, 0.02, Math.sin((i/6)*Math.PI*2)*0.9]}>
          ...
          <meshBasicMaterial color={s.color} transparent opacity={0.7} blending={THREE.AdditiveBlending} />
        </mesh>
      ))}
